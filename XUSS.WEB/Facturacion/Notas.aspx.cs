    ...
    public partial class Notas : System.Web.UI.Page
    {
        ...
        protected void rlv_notas_OnItemCommand(object sender, Telerik.Web.UI.RadListViewCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "InitInsert":
                    ...
                    try
                    {
                        tbItems = obj.GetNotaDT(null, Convert.ToString(Session["CODEMP"]), "",0);
                    }
                    catch (Exception ex)
                    {
                        throw ex;
                ...
                case "Delete":
                    ...
                    try
                    {
                        ...
                        {
                            objn.AnulaNota(null, Convert.ToString(Session["CODEMP"]), Convert.ToString(((rlv_notas.Items[0].FindControl("rc_tipfac")) as RadComboBox).SelectedValue), Convert.ToInt32(((rlv_notas.Items[0].FindControl("txt_nronota")) as RadTextBox).Text), Convert.ToString(Session["UserLogon"]));
        ...
        protected void btn_filtro_OnClick(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace((((RadButton)sender).Parent.FindControl("txt_factura") as RadTextBox).Text))
                filtro += " AND NH_NRONOTA IN (SELECT NH_NRONOTA FROM TB_NOTADT WHERE DTNROFAC =  " + (((RadButton)sender).Parent.FindControl("txt_factura") as RadTextBox).Text + ")";
        ...
        protected void rlv_notas_OnItemDataBound(object sender, RadListViewItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == RadListViewItemType.DataItem)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        tbItems = obj.GetNotaDT(null, Convert.ToString(Session["CODEMP"]), Convert.ToString((rlv_notas.Items[0].FindControl("rc_tipfac") as RadComboBox).SelectedValue), Convert.ToInt32(rlv_notas.Items[0].GetDataKeyValue("NH_NRONOTA").ToString()));
        ...
        protected void rgConsultaTerceros_OnItemCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            if (e.CommandName == "Select")
            {
                ...
                try
                {
                    ...
                    //(rlv_notas.InsertItem.FindControl("rc_sucursal") as RadComboBox).SelectedValue = fila["PHCODSUC"].ToString();
            ...
            try {
                foreach (GridDataItem item in rg_detalle.Items)
                {
                    if ((item.FindControl("chk_habilita") as CheckBox).Checked)
                    {
                        ...
                        row["ND_CANTIDAD"] = Convert.ToInt32((item.FindControl("txt_cantidaddev") as RadNumericTextBox).Value);
                        row["ND_ESTADO"] = "AC";
        ...
        protected void obj_notas_OnInserted(object sender, ObjectDataSourceStatusEventArgs e)
        {
            ...
            else
            {
                ...
                try
                {                    
                    url = "http://" + HttpContext.Current.Request.Url.Authority + "/Reportes/VisorPantallaCompleta.aspx?rpt=7013&inban=S&inParametro=inNumero&inValor=" + retorno[1] + "&inParametro=inTipo&inValor=" + retorno[0];
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "New", "window.open('" + url + "');", true);                    
                }
                catch (Exception ex)
                {
        ...
        protected void obj_notas_OnInserting(object sender, ObjectDataSourceMethodEventArgs e)
        {
            ...
            //e.InputParameters["SC_CONSECUTIVO"] = (rlv_notas.InsertItem.FindControl("rc_sucursal") as RadComboBox).SelectedValue;
        ...
        protected void rlv_notas_OnItemInserting(object sender, RadListViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                obj_notas.InsertParameters["NH_INDINV"].DefaultValue = "N";
                if ((e.ListViewItem.FindControl("chk_habilita") as CheckBox).Checked)
                    obj_notas.InsertParameters["NH_INDINV"].DefaultValue = "S";
