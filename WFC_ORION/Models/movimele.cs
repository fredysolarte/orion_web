using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace WFC_ORION.Models
{
    public class movimele
    {
        public string mecodemp { get; set; }
        public int meidmovi { get; set; }
        public int meiditem { get; set; }
        public int meidlote { get; set; }
        public int meidelem { get; set; }
        public string mefecmov { get; set; }
        public string mebodega { get; set; }
        public string metippro { get; set; }
        public string meclave1 { get; set; }
        public string meclave2 { get; set; }
        public string meclave3 { get; set; }
        public string meclave4 { get; set; }
        public string mecodcal { get; set; }
        public string mecdlote { get; set; }
        public string mecdelem { get; set; }
        public string mecdtran { get; set; }
        public double mecantid { get; set; }
        public double mecanori { get; set; }
        public double mebonifi { get; set; }
        public string meumelem { get; set; }
        public int menroele { get; set; }
        public string melocali { get; set; }
        public double? mepesobr { get; set; }
        public double? mepesont { get; set; }
        public double? meanchel { get; set; }
        public double? melargel { get; set; }
        public string medttec1 { get; set; }
        public string medttec2 { get; set; }
        public string medttec3 { get; set; }
        public string medttec4 { get; set; }
        public double? medttec5 { get; set; }
        public double? medttec6 { get; set; }
        public string meestado { get; set; }
        public string mecausae { get; set; }
        public string menmuser { get; set; }
        public string mefecing { get; set; }
        public string mefecmod { get; set; }
        public string mepdelem { get; set; }
    }
}
